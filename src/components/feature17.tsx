  ...
  return (
    <section className="py-32 bg-gradient-to-br from-blue-50 to-indigo-50">
      <div className="container mx-auto max-w-7xl px-4">
        <p className="mb-4 text-xs text-blue-600 font-semibold md:pl-5">
          {subheading}
        </p>
        <h2 className="text-3xl font-medium text-gray-900 md:pl-5 lg:text-4xl">{heading}</h2>
        <div className="mx-auto mt-14 grid gap-x-20 gap-y-8 md:grid-cols-2 md:gap-y-6 lg:mt-20">
          {features.map((feature, idx) => (
            <div className="flex gap-6 rounded-lg bg-white p-6 shadow-sm hover:shadow-md transition-shadow border border-gray-100 md:block md:p-5" key={idx}>
              <span className="mb-8 flex size-10 shrink-0 items-center justify-center rounded-full bg-blue-100 md:size-12">
                <div className="text-blue-600">
                  {feature.icon}
                </div>
              </span>
              <div>
                <h3 className="font-medium text-gray-900 md:mb-2 md:text-xl">
                  {feature.title}
                </h3>
