...
const Feature197 = ({ features = defaultFeatures }) => {
              ...
              {features.map((tab) => (
                <AccordionItem key={tab.id} value={`item-${tab.id}`}>
                  <AccordionTrigger
                    ...
                    className="cursor-pointer py-5 no-underline transition-all duration-200 hover:text-blue-600"
                  >
                    <h6
                      className={`text-xl font-semibold ${tab.id === activeTabId ? "text-blue-600" : "text-gray-600"}`}
                    >
                      {tab.title}
                    </h6>
                  </AccordionTrigger>
